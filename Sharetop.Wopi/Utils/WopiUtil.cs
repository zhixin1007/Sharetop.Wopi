...
namespace Sharetop.Wopi.Utils
{
    public static class WopiUtil
    {
        ...
        /// </summary>
        public async static Task PopulateActions(this IEnumerable<File> files)
        {
            if (files.Count() > 0)
            {
        ...
        /// </summary>
        public async static Task PopulateActions(this File file)
        {
        ...
        public async static Task<List<WopiAction>> GetDiscoveryInfo()
        {
            ...
            else
            {
                ...
                using (HttpResponseMessage response = await client.GetAsync(ServerUtil.Config().WopiDiscovery))
                {
